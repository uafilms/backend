...
app.get('/api', (req, res) => {
  res.json({ turnstile: config.turnstile, providers: config.providers });
});
...
app.get('/embed', checkTurnstile, async (req, res) => {
    ...
    const turnstileToken = req.headers['cf-turnstile-response'] || req.query.token || '';
